...
async function founderInbox(env) {
  const [approvals, blocked, messages, quota] = await Promise.all([
...
async function bootstrap(env) {
  const [agents, events, tasks, approvals, messages, schedules, usage, inbox] = await Promise.all([
...
async function route(request, env) {
  ...
  if (method === 'POST' && path === '/api/messages') {
    ...
    const inserted = await env.COMPANY_OS_DB.prepare(
      `INSERT INTO messages (sender_agent_id,recipient_agent_id,subject,body) VALUES (?,?,?,?)`
  ...
  if (method === 'POST' && path === '/api/tasks') {
    ...
    let approvalId = null;
    if (needsApproval) {
      const ap = await env.COMPANY_OS_DB.prepare(
        `INSERT INTO approvals (requested_by_agent_id,action_type,title,rationale,payload_json) VALUES (?,?,?,?,?)`
      ).bind(creator, clean(payload.action_type || 'restricted_action', 100), `Approval: ${title}`, clean(payload.description, 3000), JSON.stringify({ taskTitle: title })).run();
      approvalId = ap.meta.last_row_id;
    }
    const inserted = await env.COMPANY_OS_DB.prepare(
      `INSERT INTO tasks (title,description,owner_agent_id,created_by_agent_id,priority,due_at,approval_required,approval_id) VALUES (?,?,?,?,?,?,?,?)`
