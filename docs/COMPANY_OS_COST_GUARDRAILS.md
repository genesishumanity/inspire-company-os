...
4. **One inference per agent at a time.** A token-scoped 10-minute agent run lease prevents double-clicks, overlapping schedules, and concurrent operator requests from consuming duplicate inference for the same agent. Busy attempts defer rather than call Workers AI.
5. **Daily request soft cap.** `AI_DAILY_REQUEST_SOFT_CAP=50` Company OS AI attempts per UTC day.
6. **Quota/capacity failure = sleep/defer.** The affected agent sleeps, Founder attention is surfaced, and no paid provider is attempted.
7. **Non-quota runtime failure = blocked.** No blind retry loop.
8. **Cron work is bounded.** At most three due schedules per invocation.
9. **Scheduler reserve.** Before cron work starts, the secure entry layer requires room for all three possible runs. If `current_requests + 3 > soft_cap`, scheduled inference is deferred instead of starting work that could be falsely advanced as completed.
10. **Adaptive frontend polling.** Active changes poll at 8 seconds; after idle cycles the UI backs off to 15 then 30 seconds; hidden tabs poll at 60 seconds. Polling never invokes AI.
11. **Bootstrap read reuse.** The high-frequency bootstrap reuses its already-loaded task and approval rows to derive those Founder Inbox sections instead of issuing duplicate task/approval queries every poll.
12. **D1 hard-limit response is graceful.** If D1 reports a Free-tier daily row limit, the secure entry layer returns `503 storage_deferred` with retry-after guidance instead of pretending the operation succeeded.
13. **No heartbeat writes.** Reading the office does not create events or status noise.
...
- use the base `idx_ai_usage_ts` index with UTC range queries rather than wrapping the timestamp column in a function;
- `0003_cost_guard_indexes.sql` adds only the missing `event_type + ts` composite index used by the scheduler guard;
- do **not** duplicate existing task/approval/schedule indexes: every extra index increases D1 row writes on mutations;
- bounded recent-row queries;
- scheduler claims use a lease index added by `0004_scheduler_leases.sql`;
- per-agent AI concurrency uses columns on the primary-keyed `agents` row and needs no extra index;
- adaptive UI polling reduces repeated bootstrap reads when the office is idle;
- bootstrap derives pending approvals and blocked tasks from rows already loaded for the main UI instead of reading them twice;
- no heartbeat writes;
